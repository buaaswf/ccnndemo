...
class Pre_SegLayer(caffe.Layer):
    ...
    def reshape(self, bottom, top):
        self.depth_pred = bottom[0].data
        #  print self.depth_pred.shape
        self.depth_gt = bottom[1].data
        #  print self.depth_gt.shape
        top[0].reshape(*self.depth_gt.shape)
        top[1].reshape(*self.depth_gt.shape)
    def forward(self, bottom, top):
        ...
        for i  in range(0,256,1):
            fd, depth_pred[0][i] = hog(depth_pred[0][i], orientations=8, pixels_per_cell=(16, 16), cells_per_block=(1, 1), visualise=True)
            depth_pred[0][i] = exposure.rescale_intensity(depth_pred[0][i], in_range=(0, 0.02))
